		...
		switch( option ) {
			case 0:
				...
				*streamFile = (char*)calloc(200, sizeof(char));
				sprintf_s(*streamFile, "%s", file);
				printf("\n");
				if ( (fp = fopen_s(*streamFile, "r")) == NULL ) {
...
fileBasedBitStreams(char *streamFile)
{
	...
	if ( mode == 0 ) {
		...
		if ( (fp = fopen_s(streamFile, "rb")) == NULL ) {
			printf("ERROR IN FUNCTION fileBasedBitStreams:  file %s could not be opened.\n", streamFile);
			exit(-1);
		}
...
openOutputStreams(int option)
{
	...
	sprintf_s(freqfn, "experiments/%s/freq.txt", generatorDir[option]);
	if ( (freqfp = fopen_s(freqfn, "w")) == NULL ) {
		printf("\t\tMAIN:  Could not open freq file: <%s>", freqfn);
		exit(-1);
	}
	sprintf_s(summaryfn, "experiments/%s/finalAnalysisReport.txt", generatorDir[option]);
	if ( (summary = fopen_s(summaryfn, "w")) == NULL ) {
	...
	for( i=1; i<=NUMOFTESTS; i++ ) {
		if ( testVector[i] == 1 ) {
			sprintf_s(statsDir, "experiments/%s/%s/stats.txt", generatorDir[option], testNames[i]);
			sprintf_s(resultsDir, "experiments/%s/%s/results.txt", generatorDir[option], testNames[i]);
			if ( (stats[i] = fopen_s(statsDir, "w")) == NULL ) {	/* STATISTICS LOG */
				...
				numOfOpenFiles++;
			if ( (results[i] = fopen_s(resultsDir, "w")) == NULL ) {	/* P_VALUES LOG   */
				 printf("ERROR: LOG FILES COULD NOT BE OPENED.\n");
				 printf("       MAX # OF OPENED FILES HAS BEEN REACHED = %d\n", numOfOpenFiles);
				 printf("-OR-   THE OUTPUT DIRECTORY DOES NOT EXIST.\n");
				 exit(-1);
