...
{ 
	...
	sprintf_s(resultsDir, "experiments/%s/%s/results.txt", generatorDir[option], testNames[testNameID]);
	
	if ( (fp[numOfFiles] = fopen_s(resultsDir, "r")) == NULL ) {
		printf("%s", resultsDir);
	...
	for ( i=0; i<numOfFiles; i++ ) {
		if ( i < 10 )
			sprintf_s([i], "experiments/%s/%s/data%1d.txt", generatorDir[option], testNames[testNameID], i+1);
		else if (i < 100)
			sprintf_s(s[i], "experiments/%s/%s/data%2d.txt", generatorDir[option], testNames[testNameID], i+1);
		else
			sprintf_s(s[i], "experiments/%s/%s/data%3d.txt", generatorDir[option], testNames[testNameID], i+1);
	}
	numread = 0;
	m = numOfFiles/20;
	if ( (numOfFiles%20) != 0 )
		...
		if ( testVector[i] ) {
			...
			if ( ((i == TEST_CUSUM) && testVector[TEST_CUSUM] ) ||
				...
				for ( k=0; k<numOfFiles; k++ ) {
					if ( k < 10 )
						sprintf_s(s, "experiments/%s/%s/data%1d.txt", generatorDir[option], testNames[i], k+1);
					else if ( k < 100 )
						sprintf_s(s, "experiments/%s/%s/data%2d.txt", generatorDir[option], testNames[i], k+1);
					else
						sprintf_s(s, "experiments/%s/%s/data%3d.txt", generatorDir[option], testNames[i], k+1);
				...
				sprintf_s(s, "experiments/%s/%s/results.txt", generatorDir[option], testNames[i]);
				generalSampleSize = computeMetrics(s,i);
			}
		}
	}
